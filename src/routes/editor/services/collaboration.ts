...
export async function initCollaboration(projectId: string) {
	...
	try {
		...
		const {
			data: { user },
			error: userError
		} = await retryOperation(() => supabase.auth.getUser(), 3, 1000);
...
function handleProjectUpdate(payload: any) {
	...
	if (
		newProject.schematic_data &&
		JSON.stringify(newProject.schematic_data) !== JSON.stringify(oldProject.schematic_data)
	) {
...
export async function broadcastNodeMovement(nodeId: string, position: { x: number; y: number }) {
	...
	console.log('The node postion is brotcased', nodeId, 'x', position.x, 'y', position.y);
 ...
 */
export async function broadcastComponentAdded(
	type: string,
	position: { x: number; y: number },
	id: string
) {
 ...
 */
export async function updateCursorAction(
	action: 'idle' | 'dragging' | 'selecting' | 'drawing',
	nodeId?: string
) {
	if (!browser || !cursorChannel) return;
...
function startHealthMonitoring() {
	...
	healthCheckInterval = setInterval(() => {
		...
		if (pendingCount > MAX_PENDING_UPDATES) {
			const oldestKeys = Array.from(pendingUpdates.keys()).slice(
				0,
				pendingCount - MAX_PENDING_UPDATES
			);
