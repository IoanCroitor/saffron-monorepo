...
function createCircuitStore() {
	...
	return {
				...
				switch (type) {
					...
					case 'voltageSource':
						...
						if (sourceType === 'PULSE') {
							...
							netlist.push(`${compId} ${vNet1} ${vNet2} 0 pulse (0 ${voltage} 0 0.1 0.1 ${period/2} ${period})`);
					...
					case 'transistor':
						...
						netlist.push(`M${compId.slice(1)} ${tNetC} ${tNetB} ${tNetE} ${tNetE} ${modelName} W=100.0u L=0.09u`);
						break;
					...
					case 'opamp':
						...
						netlist.push(`X${compId.slice(1)} ${oNetPos} ${oNetNeg} ${oNetOut} ${oNetVcc} ${oNetVee} ${params.type || 'LM741'}`);
						break;
            ...
            state.edges.forEach((edge: any) => {
                const sourceCell = Object.values(cells).find((cell: any) => 
                    Object.keys(cell.connections).some(port => 
                        cell.connections[port].some((bit: number) => 
                            netMap.has(`${edge.source}_${port}`) && 
                            netMap.get(`${edge.source}_${port}`) === bit
                        )
                    )
                );
                ...
                const targetCell = Object.values(cells).find((cell: any) => 
                    Object.keys(cell.connections).some(port => 
                        cell.connections[port].some((bit: number) => 
                            netMap.has(`${edge.target}_${port}`) && 
                            netMap.get(`${edge.target}_${port}`) === bit
                        )
                    )
                );
		...
		saveChanges: async () => {
			...
			update(store => {
				if (store.projectId && store.pendingChanges) {
					...
					saveCircuitChanges(store.projectId, data)
						.then(result => {
							success = result;
							if (success) {
		...
		addComponentFromCollaborator: (type: string, position: { x: number; y: number }, id: string) => {
